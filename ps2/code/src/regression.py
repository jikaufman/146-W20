...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        # part b/g (general polynomial) implementation
    ...
    def cost(self, X, y) :
        ...
        if len(h) != len(y):
            raise Exception("cost: prediction & y have differing dimensions")
...
def main() :
    ...
    for step in [0.0001, 0.001, 0.01, 0.0407]:
        t0 = time.time()
        print "eta = " + str(step)
        model.fit_GD(train_data.X, train_data.y, eta=step, verbose=False)
        print "cost: " + str(model.cost(train_data.X,train_data.y))
        print "theta: " +  str(model.coef_)
        print "time taken: " + str(time.time() - t0) + " seconds"
        print ""

    t0 = time.time()
    model.fit(train_data.X,  train_data.y)
    print "Closed form solution: \ntheta: " + str(model.coef_)
    print "time taken: " + str(time.time() - t0) + " seconds"
    print "cost: " + str(model.cost(train_data.X,train_data.y))
    ...
    xvals = [i for i in range(11)]
    rms_vals_train = [0 for _ in range(11)]
    rms_vals_test = [0 for _ in range(11)]
    for m in range(11):
        model = PolynomialRegression(m)
        model.fit(train_data.X, train_data.y)
        rms_vals_train[m] = model.rms_error(train_data.X, train_data.y)
        rms_vals_test[m] = model.rms_error(test_data.X, test_data.y)

    plt.plot(xvals, rms_vals_train, label="Training data")
    plt.plot(xvals, rms_vals_test, label="Test data")
